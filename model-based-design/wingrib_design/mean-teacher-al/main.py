...
class ActivatorSemiSupervisor:
    ...
    def run(self, init_num=1000, add_num=500, resume_ind=None, run_steps=None):
        ...
        if resume_ind is None:
            ...
            try:
                check = torch.load('results_al/checkpoints_last.tar')
...
if __name__ == '__main__':
    ...
    params_train_val = params_dataset[train_val_ind]
    labels_train_val = labels_dataset[train_val_ind]

    mu = params_train_val.mean((0, 1, -1))
    sig = params_train_val.std((0, 1, -1))
    mu = mu.reshape(1, 1, 3, 2, 1)
    sig = sig.reshape(1, 1, 3, 2, 1)

    params_train_val = (params_train_val - mu) / sig

    MAX_NUM = 5_0000

    SEMI_SUPERVISED = True
    data_set = DataLoaderFIT(copy(params_train_val), copy(labels_train_val))
    ...
    # act.run(init_num=1000, add_num=500)
