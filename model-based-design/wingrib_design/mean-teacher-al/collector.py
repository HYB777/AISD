...
class Collector(Dataset):
    ...
    def load_state_dict(self, state_dict):
        ...
        self.dataset.scale = self.scales
    ...
    def __getitem__(self, item):
        ...
        if mode[0] == 'pool':
            ...
            return data, vm, mass, is_labeled
...
class DataLoaderFIT(Dataset):
    def __init__(self, params, labels):
        ...
        self.vm = torch.from_numpy(labels[:, 0]).float()
        self.mass = torch.from_numpy(labels[:, 1]).float()
        self.scale = torch.ones(2)
        print('total label: ', n, self.data.shape)

    def update_scale(self, data_ind):
        self.scale[0] = self.vm[data_ind].std()
        self.scale[1] = self.mass[data_ind].std()
        return self.scale
    ...
    def __getitem__(self, item):
        data = self.data[item]
        vm = self.vm[item] / self.scale[0]
        mass = self.mass[item] / self.scale[1]
