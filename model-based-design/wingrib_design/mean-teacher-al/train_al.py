...
class TrainerFitLL:
    def __init__(self, collector, opt, mu_paras, sig_paras, semi_supervised=True):
        ...
        self.best_mass_rmae = {'student': -1., 'teacher': -1.}
    ...
    def train_epoch(self, epoch):
        ...
        for block, teacher_block, student_block in train_bar:
            X, vm, mass = self.collector.dataset[block]
            ...
            X_teacher = (1 - w_teacher_noise.view(-1, 1, 1, 1, 1)) * X + w_teacher_noise.view(-1, 1, 1, 1, 1) * teacher_noise
            X_student = (1 - w_student_noise.view(-1, 1, 1, 1, 1)) * X + w_student_noise.view(-1, 1, 1, 1, 1) * student_noise
            ...
            y = torch.cat([vm.view(-1, 1), mass.view(-1, 1)], dim=1)
            ...
            vm_rmae_ = (torch.abs(y[:, 0] - y_hat_student[:, 0]).sum() / torch.abs(y[:, 0]).sum())
            mass_rmae_ = (torch.abs(y[:, 1] - y_hat_student[:, 1]).sum() / torch.abs(y[:, 1]).sum())

            total_loss += loss.item()
            vm_rmae += vm_rmae_.item()
            mass_rmae += mass_rmae_.item()
            ...
            train_bar.set_description(desc='[%d/%d] train | loss: %.4e vm: %.4e mass: %.4e step: %d size: %d #labeled: %d, consis: %.4e'
                                           % (epoch, self.opt['nEpoch'], loss.item(),
                                              vm_rmae_.item(), mass_rmae_.item(),
                                              self.global_iter, self.collector.data_size(), num_labeled, consistency_loss.item()))

        print('[%d/%d] train phase | loss=%.4e, vm=%.4e, mass=%.4e, consistency_loss=%.4e'
              % (epoch, self.opt['nEpoch'],
                 total_loss / len(blocks),
                 vm_rmae / len(blocks),
                 mass_rmae / len(blocks),
    ...
    def train_epoch_only_labeled(self, epoch):
        ...
        for X, vm, mass, is_labeled in train_bar:
            ...
            train_bar.set_description(desc='[%d/%d] train | loss: %.4e vm: %.4e mass: %.4e step: %d size: %d'
                                           % (epoch, self.opt['nEpoch'], loss.item(),
                                              vm_rmae_.item(), mass_rmae_.item(),
                                              self.global_iter, self.collector.data_size()))

        print('[%d/%d] train phase | loss=%.4e, vm=%.4e, mass=%.4e'
              % (epoch, self.opt['nEpoch'],
                 total_loss / len(train_loader),
                 vm_rmae / len(train_loader),
                 mass_rmae / len(train_loader)))
    ...
    def val_epoch(self, epoch):
        ...
        mass_rmae_u = {'student': 0., 'teacher': 0.}
        mass_rmae_d = {'student': 0., 'teacher': 0.}
        keys = ['student', 'teacher']
        for X, vm, mass, is_labeled in val_bar:
            assert torch.all(is_labeled)
            if self.opt['cuda']:
                X = X.cuda()
                vm = vm.cuda()
            ...
            mass_rmae_ = {'student': 0., 'teacher': 0.}
            ...
            for k in keys:
                ...
                vm_rmae_[k] = (torch.abs(y[:, 0] - y_hat[:, 0]).sum() / torch.abs(y[:, 0]).sum())
                mass_rmae_[k] = (torch.abs(y[:, 1] - y_hat[:, 1]).sum() / torch.abs(y[:, 1]).sum())

                vm_rmae_u[k] += torch.abs(y[:, 0] - y_hat[:, 0]).sum().item()
                vm_rmae_d[k] += torch.abs(y[:, 0]).sum().item()
                mass_rmae_u[k] += torch.abs(y[:, 1] - y_hat[:, 1]).sum().item()
                mass_rmae_d[k] += torch.abs(y[:, 1]).sum().item()

                total_loss[k] += loss.item()

            val_bar.set_description(desc='[%d/%d] val | '
                                         'student-[loss=%.4e, vm=%.4e, mass=%.4e], '
                                         'teacher=[loss=%.4e, vm=%.4e, mass=%.4e]'
                                         % (epoch, self.opt['nEpoch'],
                                            loss_item['student'], vm_rmae_['student'], mass_rmae_['student'],
                                            loss_item['teacher'], vm_rmae_['teacher'], mass_rmae_['teacher'],))

        print('[%d/%d] val phase |'
              'student-[loss=%.4e, vm=%.4e, mass=%.4e], '
              'teacher=[loss=%.4e, vm=%.4e, mass=%.4e]'
              % (epoch, self.opt['nEpoch'],
                 total_loss['student'] / len(val_loader), vm_rmae_u['student'] / vm_rmae_d['student'],
                 mass_rmae_u['student'] / mass_rmae_d['student'],
                 total_loss['teacher'] / len(val_loader), vm_rmae_u['teacher'] / vm_rmae_d['teacher'],
                 mass_rmae_u['teacher'] / mass_rmae_d['teacher']))

        for k in keys:
            mean_mae = total_loss[k] / len(val_loader)
            mean_vm_rmae = vm_rmae_u[k] / vm_rmae_d[k]
            mean_mass_rmae = mass_rmae_u[k] / mass_rmae_d[k]
            if mean_vm_rmae + mean_mass_rmae < self.best_loss[k]:
                self.best_loss[k] = mean_vm_rmae + mean_mass_rmae
                self.best_vm_rmae[k] = mean_vm_rmae
                self.best_mass_rmae[k] = mean_mass_rmae
                self.best_epoch[k] = epoch
                checkpoint_best = {
                    'epoch': epoch,
                    'loss': mean_mae,
                    'vm_rmae': mean_vm_rmae,
                    'mass_rmae': mean_mass_rmae,
                    'student': self.student.state_dict(),
                    'teacher': self.teacher.state_dict(),
    ...
    def sample_in_uncertainty(self, N=100, subset=-1):
        ...
        for t in range(N):
            ...
            for X, ind, _, teacher_noise, student_noise in bar:
                ...
                eps_teacher = torch.rand(len(X), 1, 1, 1, 1, device=X.device) * 0.1
                eps_student = torch.rand(len(X), 1, 1, 1, 1, device=X.device) * 0.1
    ...
    def update_best_score(self):
        ...
        with torch.no_grad():
            ...
            print('update phase |'
                  'student-[vm_rmae=%.4e, mass_rmae=%.4e], '
                  'teacher=[vm_rmae=%.4e, mass_rmae=%.4e]'
                  % (vm_rmae_u['student'] / vm_rmae_d['student'], mass_rmae_u['student'] / mass_rmae_d['student'],
                     vm_rmae_u['teacher'] / vm_rmae_d['teacher'], mass_rmae_u['teacher'] / mass_rmae_d['teacher'],))
            for k in keys:
                mean_vm_rmae = vm_rmae_u[k] / vm_rmae_d[k]
                mean_mass_rmae = mass_rmae_u[k] / mass_rmae_d[k]
                self.best_loss[k] = mean_vm_rmae + mean_mass_rmae
    ...
    def run(self, n_epochs, convergence_epoch=100):
        ...
        while epoch < n_epochs or self.convergence_flag < convergence_epoch:
            ...
            print('mass rmae: ', self.best_mass_rmae)
