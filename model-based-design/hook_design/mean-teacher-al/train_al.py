...
class TrainerFitLL:
    def __init__(self, collector, opt, mu_paras, sig_paras, semi_supervised=True):
        ...
        self.student = HookPhyNetFC(74)
        self.teacher = HookPhyNetFC(74)
        ...
        self.best_se_rmae = {'student': -1., 'teacher': -1.}
        self.best_vol_rmae = {'student': -1., 'teacher': -1.}
    ...
    def my_lr_schedule_step(self):
        pass
        # lr = self.opt['lr'] * (DECAY_RATE ** (self.global_iter // DECAY_STEP))
    ...
    # def warm(self, n_epoch):
    #     self.warmed = True
    #     self.student.train()
    #     self.teacher.train()
    #     self.collector.train()
    #     teacher_optimizer = optim.Adam(self.teacher.parameters(), lr=self.opt['lr'], betas=(0.9, 0.999), weight_decay=5e-5)
    ...
    #         for X, se, vol, is_labeled in train_bar:
    ...
    #                 se = se.cuda()
    #                 vol = vol.cuda()
    #             y = torch.cat([se.view(-1, 1), vol.view(-1, 1)], dim=1)
    #             y_hat = self.teacher(X)
    #             regress_loss = self.l1_loss(y - y_hat, reduction='mean')
    #             regress_loss.backward()
    #             teacher_optimizer.step()
    #             self.global_iter = self.global_iter + 1
    #     # self.teacher.ema_update(self.student, 0)

    def train_epoch(self, epoch):
        blocks, teacher_blocks, student_blocks = self.collector.train_blocks(512)
        ...
        for block, teacher_block, student_block in train_bar:
            X, se, vol = self.collector.dataset[block]
            teacher_noise, _, _ = self.collector.dataset[teacher_block]
            student_noise, _, _ = self.collector.dataset[student_block]
            ...
            X_teacher = (1 - w_teacher_noise.view(-1, 1)) * X + w_teacher_noise.view(-1, 1) * teacher_noise
            X_student = (1 - w_student_noise.view(-1, 1)) * X + w_student_noise.view(-1, 1) * student_noise
            ...
            se_rmae += se_rmae_.item()
            vol_rmae += vol_rmae_.item()
            consistency_loss_ += consistency_loss.item()

            loss.backward()
            self.optimizer.step()
            # self.lr_schedule.step()
            self.my_lr_schedule_step()
            self.teacher.ema_update(self.student, alpha)
            self.global_iter = self.global_iter + 1

            train_bar.set_description(desc='[%d/%d] train | loss: %.4e se rmae: %.4e v rmae: %.4e step: %d, size: %d, #labeled: %d, consis: %.4e'
                                           % (epoch, self.opt['nEpoch'], loss.item(), se_rmae_.item(), vol_rmae_.item(),
                                              self.global_iter, self.collector.data_size(), num_labeled, consistency_loss.item()))

        print('[%d/%d] train phase | loss=%.4e, se_rmae=%.4e, vol_rmae=%.4e, consistency_loss=%.4e'
    ...
    def train_epoch_only_labeled(self, epoch):
        ...
        for X, se, vol, is_labeled in train_bar:
            ...
            train_bar.set_description(desc='[%d/%d] train | loss: %.4e se rmae: %.4e v rmae: %.4e step: %d, size: %d'
                                           % (epoch, self.opt['nEpoch'], loss.item(), se_rmae_.item(), vol_rmae_.item(),
                                              self.global_iter, self.collector.data_size()))

        print('[%d/%d] train phase | loss=%.4e, se_rmae=%.4e, vol_rmae=%.4e'
              % (epoch, self.opt['nEpoch'],
                 total_loss / len(train_loader),
                 se_rmae / len(train_loader),
                 vol_rmae / len(train_loader)))
    ...
    def val_epoch(self, epoch):
        ...
        se_rmae_u = {'student': 0., 'teacher': 0.}
        se_rmae_d = {'student': 0., 'teacher': 0.}
        vol_rmae_u = {'student': 0., 'teacher': 0.}
        vol_rmae_d = {'student': 0., 'teacher': 0.}
        keys = ['student', 'teacher']
        for X, se, vol, is_labeled in val_bar:
            assert torch.all(is_labeled)
            if self.opt['cuda']:
                X = X.cuda()
                se = se.cuda()
            ...
            se_rmae_ = {'student': 0., 'teacher': 0.}
            vol_rmae_ = {'student': 0., 'teacher': 0.}
            ...
            for k in keys:
                ...
                se_rmae_[k] = (torch.abs(y[:, 0] - y_hat[:, 0]).sum() / torch.abs(y[:, 0]).sum())
                vol_rmae_[k] = (torch.abs(y[:, 1] - y_hat[:, 1]).sum() / torch.abs(y[:, 1]).sum())

                se_rmae_u[k] += torch.abs(y[:, 0] - y_hat[:, 0]).sum().item()
                se_rmae_d[k] += torch.abs(y[:, 0]).sum().item()
                vol_rmae_u[k] += torch.abs(y[:, 1] - y_hat[:, 1]).sum().item()
                vol_rmae_d[k] += torch.abs(y[:, 1]).sum().item()

                total_loss[k] += loss.item()

            val_bar.set_description(desc='[%d/%d] val | '
                                         'student-[loss=%.4e, se_rmae=%.4e, v_rmae=%.4e], '
                                         'teacher=[loss=%.4e, se_rmae=%.4e, v_rmae=%.4e]'
                                         % (epoch, self.opt['nEpoch'],
                                            loss_item['student'], se_rmae_['student'], vol_rmae_['student'],
                                            loss_item['teacher'], se_rmae_['teacher'], vol_rmae_['teacher'],))

        print('[%d/%d] val phase |'
              'student-[loss=%.4e, se_rmae=%.4e, v_rmae=%.4e], '
              'teacher=[loss=%.4e, se_rmae=%.4e, v_rmae=%.4e]'
              % (epoch, self.opt['nEpoch'],
                 total_loss['student'] / len(val_loader), se_rmae_u['student'] / se_rmae_d['student'],
                 vol_rmae_u['student'] / vol_rmae_d['student'],
                 total_loss['teacher'] / len(val_loader), se_rmae_u['teacher'] / se_rmae_d['teacher'],
                 vol_rmae_u['teacher'] / vol_rmae_d['teacher'],))

        for k in keys:
            mean_mae = total_loss[k] / len(val_loader)
            mean_se_rmae = se_rmae_u[k] / se_rmae_d[k]
            mean_vol_rmae = vol_rmae_u[k] / vol_rmae_d[k]
            if mean_se_rmae + mean_vol_rmae < self.best_loss[k]:
                self.convergence_flag = 0
                self.best_loss[k] = mean_se_rmae + mean_vol_rmae
                self.best_se_rmae[k] = mean_se_rmae
                self.best_vol_rmae[k] = mean_vol_rmae
                self.best_epoch[k] = epoch
                checkpoint_best = {
                    'epoch': epoch,
                    'loss': mean_mae,
                    'se_rmae': mean_se_rmae,
                    'vol_rmae': mean_vol_rmae,
                    'student': self.student.state_dict(),
                    'teacher': self.teacher.state_dict(),
    ...
    def sample_in_uncertainty(self, N=100, subset=-1):
        # student = HookPhyNetFC(74)
        # teacher = HookPhyNetFC(74)
        ...
        for t in range(N):
            ...
            for X, ind, _, teacher_noise, student_noise in bar:
                ...
                eps_teacher = torch.rand(len(X), 1, device=X.device) * 0.1
                eps_student = torch.rand(len(X), 1, device=X.device) * 0.1
    ...
    def update_best_score(self):
        ...
        with torch.no_grad():
            ...
            print('update phase |'
                  'student-[se_rmae=%.4e, v_rmae=%.4e], '
                  'teacher=[se_rmae=%.4e, v_rmae=%.4e]'
                  % (se_rmae_u['student'] / se_rmae_d['student'], vol_rmae_u['student'] / vol_rmae_d['student'],
                     se_rmae_u['teacher'] / se_rmae_d['teacher'], vol_rmae_u['teacher'] / vol_rmae_d['teacher'],))
            for k in keys:
                mean_se_rmae = se_rmae_u[k] / se_rmae_d[k]
                mean_vol_rmae = vol_rmae_u[k] / vol_rmae_d[k]
                self.best_loss[k] = mean_se_rmae + mean_vol_rmae
    ...
    def run(self, n_epochs, convergence_epoch=100):
        ...
        while epoch < n_epochs or self.convergence_flag < convergence_epoch:
            ...
            print('se rmae: ', self.best_se_rmae)
            print('vol rmae: ', self.best_vol_rmae)
