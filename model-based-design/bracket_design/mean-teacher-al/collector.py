...
class Collector(Dataset):
    ...
    def __getitem__(self, item):
        ...
        data, freq, vm, area = self.dataset[ind]

        if mode[0] == 'pool':
            rand_ind = np.random.permutation(len(self.ind_))[:2]
            noise_ind_0 = self.ind_[rand_ind[0]]
            noise_ind_1 = self.ind_[rand_ind[1]]
            teacher_noise, _, _, _ = self.dataset[noise_ind_0]
            student_noise, _, _, _ = self.dataset[noise_ind_1]
            return data, ind, is_labeled, teacher_noise, student_noise
        else:
            return data, freq, vm, area, is_labeled


class DataLoaderFIT(Dataset):
    def __init__(self, params, labels, area):
        super(DataLoaderFIT, self).__init__()

        n = len(params)
        self.data = torch.from_numpy(params).float()
        self.freq = torch.from_numpy(labels[:, 0]).float()
        self.vm = torch.from_numpy(labels[:, 1]).float()
        self.area = torch.from_numpy(area).float()
        print('total label: ', n, self.data.shape)
        assert len(self.freq) == len(self.area)
        self.scale = torch.ones(3)

    def update_scale(self, data_ind):
        self.scale[0] = self.freq[data_ind].std()
        self.scale[1] = self.vm[data_ind].std()
        self.scale[2] = self.area[data_ind].std()
        return self.scale
    ...
    def __getitem__(self, item):
        data = self.data[item]
        freq = self.freq[item] / self.scale[0]
        vm = self.vm[item] / self.scale[1]
        area = self.area[item] / self.scale[2]

        return data, freq, vm, area
