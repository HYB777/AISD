...
class TrainerFitLL:
    def __init__(self, collector, opt, mu_paras, sig_paras, semi_supervised=True):
        ...
        self.student = ResNet1d18_RMMTW(N_PARAMS)
        self.teacher = ResNet1d18_RMMTW(N_PARAMS)
    ...
    def train_epoch(self, epoch):
        ...
        for block, teacher_block, student_block in train_bar:
            X, paras, cl, cd, vol = self.collector.dataset[block]
            teacher_x_noise, teacher_paras_noise, _, _, _ = self.collector.dataset[teacher_block]
            student_x_noise, student_paras_noise, _, _, _ = self.collector.dataset[student_block]
            ...
            paras_teacher = (1 - w_teacher_noise.view(-1, 1)) * paras + w_teacher_noise.view(-1, 1) * teacher_paras_noise
            X_student = (1 - w_student_noise.view(-1, 1, 1)) * X + w_student_noise.view(-1, 1, 1) * student_x_noise
            paras_student = (1 - w_student_noise.view(-1, 1)) * paras + w_student_noise.view(-1, 1) * student_paras_noise
            ...
            if self.opt['cuda']:
                ...
                paras_teacher = paras_teacher.cuda()
                X_student = X_student.cuda()
                paras_student = paras_student.cuda()
                cl = cl.cuda()
                cd = cd.cuda()
                vol = vol.cuda()
                is_labeled = is_labeled.cuda()
            y = torch.cat([cl.view(-1, 1), cd.view(-1, 1), vol.view(-1, 1)], dim=1)
            y_hat_student = self.student(X_student, paras_teacher)
            y_hat_teacher = self.teacher(X_teacher, paras_student).detach()
            ...
            vol_rmae_ = (torch.abs(y[:, 2] - y_hat_student[:, 2]).sum() / torch.abs(y[:, 2]).sum())
            ...
            train_bar.set_description(desc='[%d/%d] train | loss: %.4e cl=%.4e cd=%.4e vol=%.4e step: %d size: %d #labeled: %d, consis: %.4e'
                                           % (epoch, self.opt['nEpoch'], loss.item(),
                                              cl_rmae_.item(), cd_rmae_.item(), vol_rmae_.item(),
                                              self.global_iter, self.collector.data_size(), num_labeled, consistency_loss.item()))

        print('[%d/%d] train phase | loss=%.4e, cl=%.4e, cd=%.4e, vol=%.4e, consistency_loss=%.4e'
    ...
    def train_epoch_only_labeled(self, epoch):
        ...
        for X, paras, cl, cd, vol, is_labeled in train_bar:
            ...
            y = torch.cat([cl.view(-1, 1), cd.view(-1, 1), vol.view(-1, 1)], dim=1)
            y_hat_student = self.student(X, paras)
            ...
            train_bar.set_description(desc='[%d/%d] train | loss: %.4e cl=%.4e cd=%.4e vol=%.4e step: %d size: %d'
                                           % (epoch, self.opt['nEpoch'], loss.item(),
                                              cl_rmae_.item(), cd_rmae_.item(), vol_rmae_.item(),
    ...
    def val_epoch(self, epoch):
        ...
        for X, paras, cl, cd, vol, is_labeled in val_bar:
            assert torch.all(is_labeled)
            if self.opt['cuda']:
                X = X.cuda()
                paras = paras.cuda()
            ...
            y = torch.cat([cl.view(-1, 1), cd.view(-1, 1), vol.view(-1, 1)], dim=1)
            y_hat_dict = {'teacher': self.teacher(X, paras), 'student': self.student(X, paras)}
            ...
            for k in keys:
                ...
                vol_rmae_[k] = (torch.abs(y[:, 2] - y_hat[:, 2]).sum() / torch.abs(y[:, 2]).sum())
                ...
                vol_rmae_u[k] += torch.abs(y[:, 2] - y_hat[:, 2]).sum().item()
                vol_rmae_d[k] += torch.abs(y[:, 2]).sum().item()

                total_loss[k] += loss.item()

            val_bar.set_description(desc='[%d/%d] val | '
                                         'student-[loss=%.4e, cl=%.4e, cd=%.4e, vol=%.4e], '
                                         'teacher=[loss=%.4e, cl=%.4e, cd=%.4e, vol=%.4e]'
                                         % (epoch, self.opt['nEpoch'],
                                            loss_item['student'], cl_rmae_['student'], cd_rmae_['student'], vol_rmae_['student'],
                                            loss_item['teacher'], cl_rmae_['teacher'], cd_rmae_['teacher'], vol_rmae_['teacher'],))
        ...
        print('[%d/%d] val phase |'
              ...
              % (epoch, self.opt['nEpoch'],
                 ...
                 cd_rmae_u['student'] / cd_rmae_d['student'], vol_rmae_u['student'] / vol_rmae_d['student'],

                 total_loss['teacher'] / len(val_loader), cl_rmae_u['teacher'] / cl_rmae_d['teacher'],
                 cd_rmae_u['teacher'] / cd_rmae_d['teacher'], vol_rmae_u['teacher'] / vol_rmae_d['teacher']))
        ...
        for k in keys:
            ...
            if mean_cl_rmae + mean_cd_rmae + mean_vol_rmae < self.best_loss[k]:
                self.best_loss[k] = mean_cl_rmae + mean_cd_rmae + mean_vol_rmae
    ...
    def sample_in_uncertainty(self, N=100, subset=-1):
        ...
        for t in range(N):
            ...
            for X, paras, ind, _, teacher_data_noise, teacher_paras_noise, student_data_noise, student_paras_noise in bar:

                if self.opt['cuda']:
                    X = X.cuda()
                    paras = paras.cuda()
                    teacher_data_noise = teacher_data_noise.cuda()
                    teacher_paras_noise = teacher_paras_noise.cuda()
                    student_data_noise = student_data_noise.cuda()
                    student_paras_noise = student_paras_noise.cuda()
                ...
                y_hat_student = self.student(
                    ...
                    (1 - eps_student.view(-1, 1)) * paras + eps_student.view(-1, 1) * student_paras_noise
                )  # (bs, dim_out)
                y_hat_teacher = self.teacher(
                    (1 - eps_teacher.view(-1, 1, 1)) * X + eps_teacher.view(-1, 1, 1) * teacher_data_noise,
                    (1 - eps_teacher.view(-1, 1)) * paras + eps_teacher.view(-1, 1) * teacher_paras_noise
                )  # (bs, dim_out)
    ...
    def update_best_score(self):
        try:
            student = ResNet1d18_RMMTW(N_PARAMS)
            teacher = ResNet1d18_RMMTW(N_PARAMS)
        ...
        with torch.no_grad():
            for X, paras, cl, cd, vol, is_labeled in val_bar:
                ...
                y = torch.cat([cl.view(-1, 1), cd.view(-1, 1), vol.view(-1, 1)], dim=1)
                y_hat_dict = {'teacher': teacher(X, paras), 'student': student(X, paras)}
                for k in keys:
                    y_hat = y_hat_dict[k]
            ...
            print('update phase |'
                  'student-[cl=%.4e, cd=%.4e, vol=%.4e], '
                  'teacher=[cl=%.4e, cd=%.4e, vol=%.4e]'
                  % (cl_rmae_u['student'] / cl_rmae_d['student'], cd_rmae_u['student'] / cd_rmae_d['student'], vol_rmae_u['student'] / vol_rmae_d['student'],
                     cl_rmae_u['teacher'] / cl_rmae_d['teacher'], cd_rmae_u['teacher'] / cd_rmae_d['teacher'], vol_rmae_u['teacher'] / vol_rmae_d['teacher'],))
            for k in keys:
                mean_cl_rmae = cl_rmae_u[k] / cl_rmae_d[k]
                mean_cd_rmae = cd_rmae_u[k] / cd_rmae_d[k]
                mean_vol_rmae = vol_rmae_u[k] / vol_rmae_d[k]
